...
const queries = [
  ...
  Voters_info.find({'history':{$regex:/GE16/}}).countDocuments(),
  Voters_info.find().sort('-last_n').limit(1),
...
Promise.all(queries)
  .then(function(results) {
    ...
    console.log('What is the last-name that comes last in the county in alphabetical order? ', results[3].map(x => x.last_n));
